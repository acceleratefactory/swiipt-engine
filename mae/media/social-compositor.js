...
import { renderComponent, COMPONENT_STATUS } from "./social-components.js";
 ...
 */
export function buildLayoutPlan(rawSpec, tokens = socialTokens()) {
  ...
  if (family === "TYPE_DOMINANT" || family === "EDITORIAL") {
    if (eyebrowCopy) put("eyebrow", "eyebrow", { text: eyebrowCopy.text, component_id: `${spec.design_id}-eyebrow` }, { height: grid * 4 });
    if (headlineCopy) put("headline", "headline", { text: headlineCopy.text, component_id: `${spec.design_id}-headline`, typography_role: family === "EDITORIAL" ? "display_headline" : "headline" }, { height: Math.round(content.height * (family === "EDITORIAL" ? 0.42 : 0.5)) });
    ...
    if (headlineCopy) add({ component_id: `${spec.design_id}-headline`, component_type: "headline", layer: "headline", x: textX, y: content.y, width: textW, height: Math.round(content.height * 0.52), alignment: spec.alignment, text: headlineCopy.text });
    ...
    if (slot) slots.push({ slot_id: slot.slot_id, x: sideX, y: content.y, width: sideW, height: content.height, fit: slot.fit ?? "cover", focal: slot.focal ?? "center", source: slot.source ?? null, required: slot.required === true });
    ...
    if (headlineCopy) put("headline", "headline", { text: headlineCopy.text, component_id: `${spec.design_id}-headline` }, { height: Math.round(content.height * 0.2) });
  ...
  const layout_plan = {
    ...
    provenance: { design_id: spec.design_id, angle_id: spec.angle_id, asset_brief_id: spec.asset_brief_id, brand_tokens_version: tokens.brand_tokens_version, token_version: tokens.token_version, compositor_version: SOCIAL_COMPOSITOR_VERSION, layout_plan_id: null },
  };
...
export function composeSocialStatic(rawSpec, tokens = socialTokens()) {
  ...
  const plan = buildLayoutPlan(spec, tokens);
  ...
  const slotPlacements = slotOnly ? (spec.visual_slots || []).map((s) => ({ ...s, x: content.x, y: content.y, width: content.width, height: content.height })) : plan.visual_slot_placements.map((s) => ({ ...s, ...(resolveSlot(s) || {}) }));
