#!/usr/bin/env node
// MAE · thin MIF ingest harness (docs/implementation/04). Wraps the EXISTING
// MarketIntelligenceService.ingest() — no new ingestion architecture.
//
// Guarantees:
//   · schema validation BEFORE persistence
//   · production scope rejects synthetic fixtures (never represented as real Market Truth)
//   · provenance / evidence_state / scan-coverage preserved as written
//   · all-or-nothing for a batch: a rejected record persists nothing (no partial writes)
//
// Usage:
//   node mae/harness/ingest-mif.mjs <file.json|dir> [more...] [--scope production|test] [--dry-run]
...
export const KIND = "mif";
export const SCHEMA = "market-intelligence-record.schema.json";
...
/**
 * Validate then persist MIF records. Validates the WHOLE batch first so a rejected record
...
export function ingestRecords(records, { scope = "production", dryRun = false } = {}) {
  ...
  for (const r of records) {
    try {
      ...
      if (scope === "production" && isFixture(r)) {
        fail(CODES.FIXTURE_NOT_PRODUCTION_SAFE, "synthetic fixture cannot be ingested as production Market Truth", { id: r.id });
      }
    } catch (e) {
  ...
  const persisted = [];
  for (const r of records) { MarketIntelligenceService.ingest(r, { scope }); persisted.push(r.id); }
  return { accepted: persisted, rejected: [], persisted };
}

if (process.argv[1] && process.argv[1].endsWith("ingest-mif.mjs")) {
  ...
  if (!paths.length) { console.error("usage: node mae/harness/ingest-mif.mjs <file|dir> [...] [--scope production|test] [--dry-run]"); process.exit(2); }
  let records;
