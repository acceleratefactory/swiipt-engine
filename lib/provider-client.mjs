  ...
  try {
    ...
    if (jsonMode) {
      try {
        ...
        return { ok: true, status: STATUS.PROVIDER_SUCCESS, model: mdl, endpoint_host: host, http_status: res.status, content, json: parsed, usage: data?.usage || null, error: null };
      } catch {
    ...
    return { ok: true, status: STATUS.PROVIDER_SUCCESS, model: mdl, endpoint_host: host, http_status: res.status, content, json: null, usage: data?.usage || null, error: null };
  } catch (e) {
    if (timer) clearTimeout(timer);
