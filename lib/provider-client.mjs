 ...
 */
/**
 * Chat Completions request with an optional structured-output capability.
 * - `jsonSchema` set  → response_format = { type:"json_schema", json_schema:{ name, strict, schema } }
 * - else `jsonMode`   → response_format = { type:"json_object" }
 * - else              → no response_format
 * A schema-mode rejection is reported explicitly; this client NEVER silently downgrades to json_object.
 */
export async function chatCompletion(opts = {}) {
  const r = await _chatCompletion(opts);
  const mode = r.structured_output_mode || (opts.jsonSchema ? "json_schema" : (opts.jsonMode ? "json_object" : "none"));
  return { ...r, structured_output_mode: mode, schema_enforcement_requested: !!opts.jsonSchema };
}
...
async function _chatCompletion({
  ...
  jsonSchema = null, schemaName = "response", strict = true,
} = {}) {
  const endpoint = chatEndpoint(baseUrl == null ? env.OPENAI_BASE_URL : baseUrl, env);
  const host = safeHost(endpoint);
  const mdl = model || resolveModel(worker, env);
  const key = apiKey || env.OPENAI_API_KEY;
  const redactEnv = { ...env, OPENAI_API_KEY: key };
  const responseFormat = jsonSchema
    ? { type: "json_schema", json_schema: { name: schemaName, strict: strict !== false, schema: jsonSchema } }
  ...
  try {
    const res = await doFetch(endpoint, {
      ...
      body: JSON.stringify({ model: mdl, temperature, ...(responseFormat ? { response_format: responseFormat } : {}), messages }),
    ...
    if (!res.ok) {
      return { ok: false, status: STATUS.PROVIDER_ATTEMPT_FAILED, model: mdl, endpoint_host: host, http_status: res.status, error: redact(`HTTP ${res.status}`, redactEnv) };
    }
    ...
    if (jsonSchema || jsonMode) {
      try {
        ...
        return { ok: true, status: STATUS.PROVIDER_SUCCESS, model: mdl, response_model: data?.model || null, endpoint_host: host, http_status: res.status, content, json: parsed, usage: data?.usage || null, error: null };
      } catch {
        return { ok: false, status: STATUS.PROVIDER_RESPONSE_INVALID, model: mdl, endpoint_host: host, http_status: res.status, error: "structured response was not parseable" };
      }
    }
    return { ok: true, status: STATUS.PROVIDER_SUCCESS, model: mdl, response_model: data?.model || null, endpoint_host: host, http_status: res.status, content, json: null, usage: data?.usage || null, error: null };
  } catch (e) {
    if (timer) clearTimeout(timer);
    const msg = e?.name === "AbortError" ? "request timed out" : (e?.message || "fetch failed");
    return { ok: false, status: STATUS.PROVIDER_ATTEMPT_FAILED, model: mdl, endpoint_host: host, http_status: null, error: redact(msg, redactEnv) };
  }
}
